...
def collect(cam_thread, arm, target):
    ...
    while True:
        ...
        # only run detection when the user actually presses enter
        if key == enter_key and (time.time() - last_capture) > 1.0:
            ...
            if detection is not None:
                ...
                if target_pose is not None:
                    ...
                    print("Enter pressed — solvePnP failed")
...
def main():
    ...
    collect_data = False # True if you want to do the hand eye calibration, False to view it
    if (collect_data == True):
        ...
        print(f"bout to view calibrated point for side {side_str}")
    ...
    def run():
        ...
        if collect_data:
            ...
            np.save("hand_eye_data.npy", np.array(data, dtype=np.float64))
        else:
            data = np.load("hand_eye_data.npy") 
        ...
        np.savetxt("../led_calibration/link3_wrt_camera.txt", link3_wrt_camera)
        if (side == RIGHT):
            np.savetxt("../live_scripts/link3_wrt_camera_right.txt", link3_wrt_camera)
            np.savetxt("../recalibrate_leds/link3_wrt_camera_right.txt", link3_wrt_camera)
        else:
            np.savetxt("../live_scripts/link3_wrt_camera_left.txt", link3_wrt_camera)
            np.savetxt("../recalibrate_leds/link3_wrt_camera_left.txt", link3_wrt_camera)
