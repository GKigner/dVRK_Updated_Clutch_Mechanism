...
import matplotlib
matplotlib.use("TkAgg")          # falls back to whatever's available if Tk isn't
...
if (side == RIGHT):
    TARGET_FOLDER     = './led_data_right/'
    OUTPUT_LABELS     = './led_labels.json'
    CALIBRATION_FILE  = '../common_data/led_calibration_right.json'   # consumed by fixed_roi.py
elif (side == LEFT):
    TARGET_FOLDER     = './led_data_left/'
    OUTPUT_LABELS     = './led_labels.json'
    CALIBRATION_FILE  = '../common_data/led_calibration_left.json'   # consumed by fixed_roi.py
else:
...
if (side == RIGHT):
    all_images = glob.glob('led_data_right/frames/frame_*.png')
else:
    all_images = glob.glob('led_data_left/frames/frame_*.png')

N_TO_LABEL    = 35 #len(all_images)
print (N_TO_LABEL)
...
def main():
    ...
    # ---- write calibration file consumed by fixed_roi.py ----
    calib = {
    ...
    if (side == RIGHT):
        with open('../common_data/led_calibration_right.json', 'w') as f:
            json.dump(calib, f, indent=2)
    else:
        with open('../common_data/led_calibration_left.json', 'w') as f:
            json.dump(calib, f, indent=2)
